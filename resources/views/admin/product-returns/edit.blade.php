                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Edit Product Return</h3>
                            </div>
                            {{ Form::open(['url' => route('admin.product-returns.update',$model->id), 'method' => 'POST','enctype'=> 'multipart/form-data']) }}
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    {{Form::label('Location')}}
                                    {{Form::text('location' , $model->location , ['class' => 'form-control form-control-user' ,'placeholder'=>'Kathmandu' ,'required' => 'required'])}}
                                </div>
                                <div class="form-group">
                                    {{Form::label('Phone')}}
                                    {{Form::number('phone' , $model->phone , ['class' => 'form-control form-control-user' ,'placeholder'=>'988776654' ,'required' => 'required'])}}
                                </div>
                                <div class="form-group">
                                    {{Form::label('Company')}}
                            ...
                            <div class="card-text">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                    @foreach($productReturnDetails as $productReturnDetail)
                                        <tr>
                                            <td>{{$productReturnDetail->products?$productReturnDetail->products->name:''}}</td>
                                            <td>{{$productReturnDetail->category?$productReturnDetail->category->title:''}}</td>
                                            <td>{{$productReturnDetail->subCategory?$productReturnDetail->subCategory->title:''}}</td>
                                            <td>{{$productReturnDetail->productBatch?$productReturnDetail->productBatch->name:''}}</td>
                                            <td>{{$productReturnDetail->quantity}}</td>
                                            <td>{{$productReturnDetail->alt_quantity}}</td>
                                        </tr>
                                    @endforeach
